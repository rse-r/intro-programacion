...
En la figura se ve un árbol de directorios: en la raíz del árbol se encuentra la unidad `C:` (que suele corresponder al disco rígido de la máquina) y a partir de ahí se ramifican los distintos directorios y archivos. `Documentos` y `Downloads` son dos carpetas que están en el directorio *raíz*. Las carpetas `proyecto`, `Fotos` y `Libros` están dentro de la carpeta `Documentos`. Dentro de la carpeta `proyecto` se encuentran tres archivos: `proceso.R` y `grafico.png` y un subdirectorio (o subcarpeta) llamado `datos`, que contiene el archivo `datos.csv`. Además, dentro de la carpeta `Fotos` se encuentra el archivo `vacaciones.jpg`.
...
- la ruta relativa `../Fotos/vacaciones.jpg` se refiere al archivo `vacaciones.jpg` que se encuentra en la carpeta `Fotos`, que es un nivel más arriba en la jerarquía de directorios (los 2 puntos `../` refieren a la carpeta que está un nivel más arriba).
...
Teniendo en cuenta el árbol de directorios de ejemplo, escribe: 

1. La ruta *absoluta* de `datos.csv` y de `vacaciones.jpg`.
2. La ruta *relativa* a `vacaciones.jpg` si el directorio de trabajo es `analisis`.
3. La ruta relativa a datos.csv si el directorio de trabajo es Documentos.
4. Qué significa ../ y en qué caso lo usarías.
...
* **Mayúsculas y minúsculas**: En Windows no importa, pero en Mac/Linux sí. Es recomendable respetar siempre las mayúsculas y minúsculas al escribir nombres de archivos y carpetas.
...
::: ejercicio
Construir una jerarquía de carpetas

Teniendo en cuenta esta estructura de proyecto:
...
1. Creala en el disco rigido de tu computadora y respetando las buenas practicas en nombres de archivos (minúsculas, sin espacios ni tildes, guiones como separadores). 

2. Hace una captura de pantalla y subila al campus. 
