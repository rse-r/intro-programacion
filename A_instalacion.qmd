...
## Configurando Git, GitHub y RStudio 

Para que Git, GitHub y RStudio trabajen juntos hay que presentarlos. Vamos a seguir los siguientes pasos:

1. Dale tus credenciales a git ejecutando el siguiente codigo desde la consola de R, sustituyendo el nombre y el correo electrónico por tu nombre y el correo electrónico que utilizaste para registrarte en GitHub
...
2. Ahora tenemos que generar un token de acceso personal (Personal access token (PAT) para que RStudio pueda acceder a GitHub. Ejecuta el siguiente codigo en la consola de R:

``` r
usethis::create_github_token()
```

3. Asumiendo que estas logueado en GitHub, el codigo abrira una nueva ventana en tu navegador de internet para que puedas crear un nuevo PAT. En la pagina de GitHub, haz click en "Generate new token" y en la siguiente pagina selecciona los permisos que necesitas.  No cierres esta ventana porque tendrás que copiar el token en el portapapeles. 

4. El siguiente paso es configurar RStudio para que use el token que acabas de crear. Para eso, ejecuta el siguiente codigo en la consola de R:

``` r
gitcreds::gitcreds_set()
```

5. Si aún no tienes un PAT almacenado, te pedirá que introduzcas tu PAT. Ve a la ventana anterior, copia el token, pégalo en tu consola y pulsa enter.
...
6. Por último, para comprobar que todo va bien con este comando

``` r
useethis::gh_token_help()
```

Debería imprimir una lista de detalles personales, incluyendo:
