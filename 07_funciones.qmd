...
* Si por alguna razón tenés que cambiar el código, sólo es necesitás hacerlo en un úncio lugar.
* Eliminás la posibilidad de cometer errores al copiar y pegar código.

En las situaciones donde nuestro código no produce resultados, más bien son definiciones de funciones secundarias u otras herramientras, no tiene tanto sentido usar archivos .Rmd o .qmd. En estos casos podemos volver a los tradicionales scripts.

## Esqueleto de una función

Cualquier función en R tendrá la siguiqnete pinta:
...
Está función está lista para usar, por ejemplo podemos convertir 32 Fahrenheit a centigrados:

```{r}
fahrenheit_a_centigrados(100)
```

Si bien podemos nombrar a los argumentos de las funciones de cualquier manera, `temperatura_ayer` no razonable para una función general. Un mejor nombre podría ser `temperatura_centigrados`
...
```{r}
fahrenheit_a_centigrados <- function(temperatura_centigrados) {
  resultado <- (temperatura_centigrados - 32) * 5/9
  return(resultado)
}
```
...
También es importante testear la función con datos diferentes, por ejemplo en vez de 1 enero, podemos usar un real o un vector de números.
